...
@api_bp.route("/wa/webhook", methods=["GET", "POST"])
def wa_webhook():
    """Meta WA Cloud API webhook (verification + inbound)."""
    ...
    # POST: inbound message (log only for MVP)
    data = request.get_json(silent=True) or {}
    current_app.logger.info(f"WA inbound: {data}")
